function phi = SimulationSingle(NRun, spx, spy, spz, ssx, ssy, ssz, sdx, sdy, sdz, iso, se, sse, hx, hy, hz, nx, ny, nz, ecut, e0, e1, sigmaa0, sigmaa1, sigmas0, sigmas1, g0, g1, alpha)
    phi = zeros(nx, ny, nz);
    cdx = 2*hx/nx;
    cdy = 2*hy/ny;
    cdz = 2*hz/nz;
    for iRun = 1 : NRun
        ...
        while (abs(px) <= hx && abs(py) <= hy && abs(pz) <= hz && isAbso ==0 && e > ecut)
            ...
            phi(ix, iy, iz) = phi(ix, iy, iz) + 1/(sigmaa0+sigmas0);
%             e = et;
